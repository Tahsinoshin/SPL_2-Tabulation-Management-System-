...
if(isset($_POST['upload_marks']))
    {
		...
		$tempFileName = $_FILES["marked_file"]["tmp_name"];
		...
		*/ 
		...
		if($result) { 
			...
			$query = "UPDATE modified_files SET filepath='$target', filename='$revised_file', course_type='$course_type' where faculty_email='$email' ";//VALUES ('$fileTarget','$fileName','$fileDescription')
			$db_conn->query($query) or die("Error : ".mysqli_error($db_conn));
            
            echo "Your file <html><b><i>".$revised_file."</i></b></html> has been successfully uploaded!";
		}
		else {			
